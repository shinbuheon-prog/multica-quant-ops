...
def build_report_workflow() -> DailyWorkflowService:
...
def test_daily_report_includes_successful_order_summary() -> None:
    service = build_report_workflow()
    ...
    assert "Blocked stage: none" in report
    assert "Signal: long" in report
    assert "Paper order: buy 2 AAPL" in report


def test_daily_report_includes_block_reason() -> None:
    service = build_report_workflow()
    now = datetime(2026, 4, 13, 9, 35, 0)
    request = DailyWorkflowRequest(
        symbol="AAPL",
    ...
    assert "Blocked stage: data_quality" in report
    assert "Data quality reasons: snapshot is stale" in report
    assert "Paper order: not created" in report
